...
import { S3Client, CreateMultipartUploadCommand, GetObjectCommand, AbortMultipartUploadCommand } from '@aws-sdk/client-s3'
...
export async function POST(request: NextRequest) {
  try {
    ...
    for (let partNumber = 1; partNumber <= totalChunks; partNumber++) {
      // 注意：这里我们使用 S3 的 PutObjectCommand 来生成预签名URL
      // 但实际上我们需要为分块上传生成特定的URL
      // 由于AWS SDK的限制，我们需要手动构建预签名URL
      
      // 手动构建预签名URL
      const url = new URL(`${process.env.R2_ENDPOINT_URL}/${process.env.R2_BUCKET_NAME}/${key}`)
      
      // 添加查询参数
      url.searchParams.set('partNumber', partNumber.toString())
      url.searchParams.set('uploadId', UploadId)
      
      // 这里我们需要手动签名，但为了简化，我们将使用另一种方法
      // 在实际应用中，你可能需要使用AWS SDK的签名功能
      
      uploadUrls.push({
        partNumber,
        url: url.toString(),
        // 注意：在实际应用中，你需要添加签名参数
        // 这里我们返回一个基本URL，客户端需要使用AWS SDK进行签名
      })
    }
