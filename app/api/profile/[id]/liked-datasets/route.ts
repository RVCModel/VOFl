...
export async function GET(
  ...
  try {
    ...
    // 获取用户点赞的数据集
    const { data: likedDatasetsData, error: likedDatasetsError } = await supabase
      .from('dataset_likes')
      .select(`
        dataset_id,
        created_at,
    ...
    if (likedDatasetsError) {
      console.error('获取点赞数据集列表失败:', likedDatasetsError)
    ...
    // 提取数据集数据
    const datasets = likedDatasetsData?.map(item => item.datasets) || []
