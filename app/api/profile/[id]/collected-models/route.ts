...
export async function GET(
  ...
  try {
    ...
    // 获取用户收藏的模型
    const { data: collectedModelsData, error: collectedModelsError } = await supabase
    ...
    if (collectedModelsError) {
      console.error('获取收藏模型列表失败:', collectedModelsError)
    ...
    // 提取模型数据
    const models = collectedModelsData?.map(item => item.models) || []
