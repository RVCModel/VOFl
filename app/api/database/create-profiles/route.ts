...
export async function POST() {
  try {
    ...
    const createProfilesSQL = `
      ...
      -- profiles表的RLS策略
      DROP POLICY IF EXISTS "用户可以查看所有公开的配置文件" ON public.profiles;
      CREATE POLICY "用户可以查看所有公开的配置文件" ON public.profiles
        FOR SELECT USING (true);

      DROP POLICY IF EXISTS "用户可以插入自己的配置文件" ON public.profiles;
      CREATE POLICY "用户可以插入自己的配置文件" ON public.profiles
        FOR INSERT WITH CHECK (auth.uid() = id);

      DROP POLICY IF EXISTS "用户可以更新自己的配置文件" ON public.profiles;
      CREATE POLICY "用户可以更新自己的配置文件" ON public.profiles
        FOR UPDATE USING (auth.uid() = id);

      DROP POLICY IF EXISTS "管理员可以更新所有配置文件" ON public.profiles;
    ...
    if (error) {
      console.error('创建profiles表失败:', error)
      return NextResponse.json({ error: error.message }, { status: 500 })
    }

    return NextResponse.json({ message: 'profiles表创建成功' })
  } catch (error: any) {
